...
// دليل الأسماء والأرقام المشترك: كل رقم يرتبط باسم محدد، بدون استخدام أسماء افتراضية.
...
function getCloudContactsDirectory() {
    const out = [];
    const list = _jsonBinRecord && Array.isArray(_jsonBinRecord.contacts_directory) ? _jsonBinRecord.contacts_directory : [];
    list.forEach(c => { if (c && c.phone && c.name) out.push(c); });
    if (_jsonBinRecord && _jsonBinRecord.merchant_phone && _jsonBinRecord.merchant_name) out.push({name:_jsonBinRecord.merchant_name, phone:_jsonBinRecord.merchant_phone, type:'merchant'});
    if (_jsonBinRecord && _jsonBinRecord.recipient_phone && _jsonBinRecord.recipient_name) out.push({name:_jsonBinRecord.recipient_name, phone:_jsonBinRecord.recipient_phone, type:'recipient'});
    return out;
}
...
function findContactByPhone(phone) {
    ...
    return getCloudContactsDirectory().find(c => normalizePhone(c.phone) === target) ||
           getContactsDirectory().find(c => normalizePhone(c.phone) === target) || null;
}

async function findContactByPhoneFresh(phone) {
    const target = normalizePhone(phone);
    const localResult = findContactByPhone(phone);
    if (!target || target === '0') return null;
    try {
        const record = await jsonBinRead();
        const list = Array.isArray(record.contacts_directory) ? record.contacts_directory : [];
        let match = list.find(c => c && c.name && normalizePhone(c.phone) === target);
        if (!match && record.merchant_phone && record.merchant_name && normalizePhone(record.merchant_phone) === target) match = {name:record.merchant_name, phone:record.merchant_phone, type:'merchant'};
        if (!match && record.recipient_phone && record.recipient_name && normalizePhone(record.recipient_phone) === target) match = {name:record.recipient_name, phone:record.recipient_phone, type:'recipient'};
        if (match) { _jsonBinRecord = record; return match; }
    } catch(e) { console.warn('JSONBin contact lookup:', e); }
    return localResult;
}

function saveContactMapping(name, phone, type) {
    const cleanName = String(name || '').trim();
    const cleanPhone = String(phone || '').trim();
    const normalized = normalizePhone(cleanPhone);
    if (!cleanName || !normalized || normalized === '0') return;

    const list = getContactsDirectory();
    const idx = list.findIndex(c => normalizePhone(c.phone) === normalized);
...
async function loadSettingsFromDB() {
    try {
        ...
        oldContacts.forEach(c => {
            ...
            const idx = migratedContacts.findIndex(x => normalizePhone(x.phone) === n);
...
function saveLeftData(isAuto = false) {
    ...
    saveContactMapping(mName, mPhone, 'merchant');
    saveContactMapping(rName, rPhone, 'recipient');
