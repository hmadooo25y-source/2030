...
// بحث شامل مخصص لشاشة "تاجر من جهات الاتصال-جديد":
// يغطي قائمة التجار السحابية (record.merchants_list) + جهات الاتصال السحابية العامة
// + قائمة التجار المحلية (merchants_list) + دليل جهات الاتصال المحلي + المرجع المخفي كحل أخير.
async function findMerchantByPhoneFresh(phone) {
    const target = phoneKey(phone);
    if (!target) return null;
    ...
    try {
        ...
        // 1) قائمة التجار السحابية أولاً (الأولوية للتجار).
        const cloudMerchantList = Array.isArray(record?.merchants_list) ? record.merchants_list : [];
        const cloudMerchantMatch = findInContacts(cloudMerchantList, target);
        if (cloudMerchantMatch) return cloudMerchantMatch;

        // 2) بقية جهات الاتصال السحابية (contacts + contacts_directory).
        const cloudContactMatch = findInContacts(collectCloudContacts(record), target);
        if (cloudContactMatch) return cloudContactMatch;

        // 3) قائمة التجار المحلية.
        const localMerchantMatch = findInContacts(getLocalMerchants(), target);
        if (localMerchantMatch) return localMerchantMatch;

        // 4) الدليل المحلي العام.
        const localDirMatch = findInContacts(getContactsDirectory(), target);
        if (localDirMatch) return localDirMatch;

        // 5) وأخيراً المرجع المخفي كحل أخير فقط.
        return findInHiddenReferences(target);
    } catch (e) {
        console.warn('Merchant lookup failed, falling back to local data:', e);
        return findInContacts(getLocalMerchants(), target) ||
               findInContacts(getContactsDirectory(), target) ||
               findInHiddenReferences(target);
    }
}
...
async function checkAndGoToS9(e) {
    ...
    const contact = await findMerchantByPhoneFresh(phoneVal);
