...
function findContactByPhone(phone) {
    ...
    return getContactsDirectory().find(c => normalizePhone(c.phone) === target) || null;
}
...
function checkAndGoToS9(e) {
    ...
    const contact = findContactByPhone(phoneVal);
...
merchantPhoneField?.addEventListener('blur', () => {
...
fileInput?.addEventListener('change', async (e) => {
    ...
    try {
        ...
        const contact = findContactByPhone(decodedText.trim());
        if (!contact || !contact.name) {
            showVerifyModal();
            stopScanner();
            return;
...
if (nextBtnS9) {
    nextBtnS9.addEventListener('click', (e) => {
        ...
        if (!merchantContact || !merchantContact.name) { showVerifyModal(); return; }
