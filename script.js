...
(function initLoginMenu() {
    ...
    const triggerIcon = document.getElementById('login-menu-trigger-icon');
    ...
    const openMenu = () => {
        ...
        if (triggerIcon) {
            ...
            triggerIcon.classList.add('fa-arrow-right');
        }
        trigger.setAttribute('aria-label', 'إغلاق القائمة');
    };
    ...
    const closeMenu = () => {
        ...
        if (triggerIcon) {
            triggerIcon.classList.remove('fa-arrow-right');
            triggerIcon.classList.add('fa-bars');
        }
        trigger.setAttribute('aria-label', 'فتح القائمة');
    };
    ...
    // العناصر الخمسة مقصودة كعناصر ثابتة فقط: لا فتح صفحات، لا انتقال، ولا تأثير ضغط.
